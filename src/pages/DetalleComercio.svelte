<script>
  ...
  async function handleVerificar() {
    ...
    try {
      ...
      showToast('Error al verificar. Intent芍 de nuevo.')
    } finally {
      verificando = false
    }
  }

  // Reclamo con c車digo privado
  ...
  <main class="detalle-main">
    ...
    {:else if comercio}
        ...
        <div class="hero-info">
          ...
          {#if distancia !== null}
            <p class="hero-dist">
              ...
              {formatDistancia(distancia)} de tu ubicaci車n
            </p>
          {/if}
        </div>
      </div>
      ...
      <div class="info-section">
        ...
        {#if comercio.direccion}
          <button class="info-row" on:click={() => abrirMapa(comercio.lat, comercio.lng, comercio.nombre + ' ' + comercio.direccion)}>
            ...
            <div class="info-text">
              <span class="info-label">Direcci車n</span>
        ...
        {#if comercio.descripcion}
          <div class="info-row no-action">
            ...
            <div class="info-text">
              <span class="info-label">Descripci車n</span>
              <span class="info-value">{comercio.descripcion}</span>
            </div>
          </div>
        {/if}

        <!-- Estado de verificaci車n -->
        <div class="info-row no-action">
          <div class="info-icon-wrap">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--c-primary)" stroke-width="2">
          ...
          <div class="info-text">
            ...
            <span class="estado-chip" style="color:{estadoInfo.color};background:{estadoInfo.bg}">
              ...
              {#if comercio.totalVerificaciones > 0}
                ﹞ {comercio.totalVerificaciones} verificaci車n{comercio.totalVerificaciones !== 1 ? 'es' : ''}
              {/if}
            </span>
          </div>
        </div>
      ...
      <div class="acciones-section">
        <h3 class="acciones-titulo">?Conoc谷s este comercio?</h3>
        ...
        {:else}
          <button class="btn-verificar" on:click={handleVerificar} disabled={verificando}>
            ...
            {:else}
              ...
              Confirmar que existe en esta direcci車n
            {/if}
          </button>
          <p class="verificar-desc">
            Al verificar confirm芍s que este comercio existe y est芍 en la direcci車n indicada. Con 3 verificaciones queda confirmado autom芍ticamente.
          </p>
        {/if}
        ...
        {#if !comercio.reclamadoPor && user && !comercio.reclamoBloqueado}
          {#if !mostrarFormReclamo}
            <button class="btn-reclamar" on:click={toggleFormReclamo}>
              ...
              Soy el due?o 〞 reclamar gesti車n
            </button>
          {:else}
            <div class="reclamo-form">
              <p class="reclamo-desc">
                Ingres芍 el c車digo de 6 caracteres que figura en la credencial f赤sica entregada por el administrador.
              </p>
              <input
                class="reclamo-input"
        ...
        {:else if comercio.reclamoBloqueado}
          <div class="reclamo-bloqueado">
            ?? Este comercio est芍 bloqueado por intentos fallidos. Contact芍 al administrador.
          </div>
